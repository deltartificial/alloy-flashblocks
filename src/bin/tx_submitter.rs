...
async fn test_flashblocks_methods(provider: &impl Provider) -> Result<()> {
    ...
    if let Some(block) = pending_block.get("result") {
        ...
        info!("  Number: {}", block.get("number").unwrap_or(&serde_json::json!("unknown")));
        info!("  Hash: {}", block.get("hash").unwrap_or(&serde_json::json!("unknown")));
    ...
    let balance: serde_json::Value = provider
        .raw_request(method.into(), serde_json::json!([format!("{test_address:?}"), "pending"]))
        .await?;
    
    info!("Balance for {test_address:?}:");
    info!("  Raw: {}", balance.get("result").unwrap_or(&serde_json::json!("0x0")));
    ...
    while tokio::time::Instant::now() < end_time {
        ...
        if current_block > start_block {
            ...
            if let Some(block) = block.get("result") {
                info!("  Timestamp: {}", block.get("timestamp").unwrap_or(&serde_json::json!("unknown")));
                info!("  Gas Used: {}", block.get("gasUsed").unwrap_or(&serde_json::json!("unknown")));
                if let Some(txs) = block.get("transactions").and_then(|t| t.as_array()) {
